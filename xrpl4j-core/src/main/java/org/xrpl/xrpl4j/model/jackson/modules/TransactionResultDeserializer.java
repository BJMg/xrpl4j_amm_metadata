...
public class TransactionResultDeserializer<T extends Transaction> extends StdDeserializer<TransactionResult<T>> {
    ...
    private static final Logger log = LoggerFactory.getLogger(TransactionResultDeserializer.class);
    ...
    public TransactionResult<T> deserialize(JsonParser jsonParser, DeserializationContext ctxt) throws IOException {
        ...
        try {
            ...
            transaction = objectMapper.convertValue(objectNode, javaType);
        } catch (Exception e) {
            log.error("Cannot deserialize transaction. Deserializing to Unknown.", e);
            transaction = (T) objectMapper.convertValue(objectNode, Unknown.class);
        }
